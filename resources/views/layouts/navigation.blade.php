            ...
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center" style="text-decoration: none;">
                        <img src="{{ asset('bien.jpeg') }}" alt="Logo" style="height: 50px; width: auto;" class="rounded shadow-sm">
                        <span class="ms-3 fw-bold text-gray-800 fs-4">TriDate</span>
                    </a>
                </div>
            </div>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="GET" action="{{ route('login') }}">
                            @csrf
                            <x-dropdown-link :href="route('login')" onclick="event.preventDefault(); this.closest('form').submit();">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link href="#">{{ __('Profile') }}</x-responsive-nav-link>
                <form method="GET" action="{{ route('login') }}">
                    @csrf
